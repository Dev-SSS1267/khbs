<template>
    ...
    <section class="mb-10 max-w-screen-md mx-auto">
        <div v-if="issues.length > 0">
            <div v-for="issue in issues" :key="issue._id" class="my-4 p-4 bg-gray-800 rounded">
                <h3 class="text-xl font-semibold mb-2">{{ issue.title }}</h3>
                <p>{{ issue.description }}</p>
                <p class="text-sm text-gray-500 mt-1">{{ formatDate(issue.createdAt) }}</p>
                ...
                <div v-if="activeIssueId === issue._id" class="my-4 p-4 bg-gray-700 rounded">
                    ...
                    <button @click="submitResponse(issue._id)" class="bg-green-500 text-white px-4 py-2 rounded">
                        등록하기
                    </button>
                </div>
...
export default {
    ...
    methods: {
        ...
        async submitResponse(issueId) {
            try {
                await api.post(`/issues/${issueId}/response`, { response: this.responseText });
